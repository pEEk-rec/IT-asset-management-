...
const LoginPage = () => {
  ...
  const loginSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      console.log(data);
      // Backend returns data.user.role, not data.userType
      // Also ensure we handle cases where token is returned (which contains role)

      // Store token in cookie if needed by other components (getUserType relies on 'Authtoken' cookie)
      if (data.token) {
        document.cookie = `Authtoken=${data.token}; path=/; max-age=86400`; // 24h
      }

      const userRole = data.user ? data.user.role : (jwtDecode(data.token).role);
